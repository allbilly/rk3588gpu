...
def mesa_vmul(
    ...
    *,
    ...
    return replay_events(
        steps_to_tuples(steps, inputs=inputs), verbose=verbose, trace=trace
    )
...
class _StageTrace:
    ...
    def skip(self, idx: int, nr: int, req: int, reason: str) -> None:
        ...
        self.section_event_count[self.stage_id] += 1
        self.logical_event_count[stage] += 1
        print(f"  [S{stage_id}.{self.stage_step}] idx={idx} {_ioctl_name(nr)} req=0x{req:08x} skipped ({reason})", flush=True)
    ...
    def finish(self, *, ok: bool) -> None:
        ...
        for section_id in self.section_order:
            print(
                f"  [S{section_id}] {self.section_label[section_id]:<32} "
                f"events={self.section_event_count.get(section_id, 0):03d} time_ms={self.section_total_ms.get(section_id, 0.0):.2f}",
                flush=True,
            )
...
def main() -> int:
    ...
    if args.dry_run:
        ...
        return 0
    try:
        return mesa_vmul(INPUT_A, INPUT_B, verbose=args.verbose, trace=True)
    except OSError as exc:
